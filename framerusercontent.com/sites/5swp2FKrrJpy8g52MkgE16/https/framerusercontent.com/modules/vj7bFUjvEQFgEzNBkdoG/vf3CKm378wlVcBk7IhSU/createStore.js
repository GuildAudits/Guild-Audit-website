...
export function createStore(state1) { // Use Data so that a Preview reload resets the state
    const dataStore = Data({
        state: Object.freeze({ ...state1
        })
    ...
    const setDataStore = newState => { // If the state is an object, make sure we copy it
        ...
        dataStore.state = Object.freeze({ ...dataStore.state,
            ...newState
        });
    }; // Store the initial state, copy the object if it's an object
    let storeState = typeof state1 === "object" ? Object.freeze({ ...state1
    }) : state1; // Keep a list of all the listeners, in the form of React hook setters
    const storeSetters = new Set(); // Create a set function that updates all the listeners / setters
    const setStoreState = newState => { // If the state is an object, make sure we copy it
        if (typeof newState === "function") {
            newState = newState(storeState);
        }
        storeState = typeof newState === "object" ? Object.freeze({ ...storeState,
            ...newState
    ...
    function useStore() { // Create the hook we are going to use as a listener
        ...
        } else { // Return the state and a function to update the central store
