...
import {
    createStore
} from "https://framer.com/m/framer/store.js@^1.0.0"; // Learn more: https://www.framer.com/docs/guides/overrides/
...
const changeTheme = theme => {
    ...
    if (theme === "system") {
        localStorage.getItem("currentToggleState") && localStorage.removeItem("currentToggleState");
    } else {
        localStorage.setItem("currentToggleState", `${theme}`);
    }
    const event = new Event("themeChange");
    window.dispatchEvent(event); // console.log(event)
    return;
};
export function withToggleTheme(Component) {
    return props => {
        const [store, setStore] = useStore(); // Detect theme and setup stylesheets on mount
        useEffect(() => {
            const updateTheme = () => {
                const prefersDarkScheme = window.matchMedia("(prefers-color-scheme: dark)");
                const prefersLightScheme = window.matchMedia("(prefers-color-scheme: light)"); // If true set theme to system otherwise set it to light
                const detectedTheme = prefersDarkScheme.matches || prefersLightScheme.matches ? "system" : "light"; // Store the theme
                setStore({
                    theme: `${detectedTheme}`
                });
            };
            updateTheme(); // Create attributes on html and body so that theme will be applied based on store
            const htmlElement = document.getElementsByTagName("html")[0];
            const bodyElement = document.getElementsByTagName("body")[0];
            ...
            for (let i = 0; i < document.styleSheets.length; i++) {
                ...
                try {
                    for (let rule of sheet.cssRules) { // Get light and dark mode tokens
...
export function withLightTheme(Component) {
    return props => {
        const [store, setStore] = useStore(); // Handle the click
...
export function withDarkTheme(Component) {
    return props => {
        const [store, setStore] = useStore();
        const clickDarkTheme = () => { // Storing the mode
            setStore({
                theme: "dark"
            });
            changeTheme("dark");
...
export function withSystemTheme(Component) {
    return props => {
        const [store, setStore] = useStore();
        const clickSystemTheme = () => {
            if (store.theme === "system") {
                return;
            } else { // Store the theme choice
                setStore({
                    theme: "system"
                });
                changeTheme("system");
...
//# sourceMappingURL=./ThemeToggle.map